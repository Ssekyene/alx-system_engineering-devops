...
[5-mysql_backup](./5-mysql_backup) is Bash script that generates a MySQL dump (`backup.sql`) and creates a compressed archive (`day-month-year.tar.gz`) out of it using the `root` user. The script accepts one argument that is the password used to connect to the MySQL database.
