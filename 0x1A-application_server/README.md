...
Let's serve what you built for [AirBnB clone v2 - Web framework](https://github.com/Ssekyene/AirBnB_clone_v2) on `web-01`. This task is an exercise in setting up your development environment, which is used for testing and debugging your code before deploying it to production.
...
To check for listening ports and in order to validate whether port `5000` is free use:
```
sudo netstat -lpn
```
...
Incase port `5000` is already actively listening, you may terminate it by using its PID forexample:
```
sudo kill <PID>
```
...
Notes:
- The file [2-app_server-nginx_config](./2-app_server-nginx_config) is a copy of `Nginx` config file.
...
The file [3-app_server-nginx_config](./3-app_server-nginx_config) is a copy of Nginx config file for this task.
...
Let’s serve what you built for [AirBnB clone v3 - RESTful API](https://github.com/Ssekyene/AirBnB_clone_v3) on `web-01`.
...
- It may be helpful to import your data (and environment variables) from [this project](https://github.com/Ssekyene/AirBnB_clone_v2)

The file [4-app_server-nginx_config](./4-app_server-nginx_config) is a copy of the Nginx config file for this task.
...
Let’s serve what you built for [AirBnB clone - Web dynamic](https://github.com/Ssekyene/AirBnB_clone_v4) on `web-01`.
...
The file [gunicorn.service](./gunicorn.service) is a copy of the systemd unit file
```
...
The file [4-reload_gunicorn_no_downtime](./4-reload_gunicorn_no_downtime) is a Bash script to reload Gunicorn in a graceful way.
...
- [How to Serve a Flask Application with Gunicorn and Nginx on Ubuntu 16.04](https://www.digitalocean.com/community/tutorials/how-to-serve-flask-applications-with-gunicorn-and-nginx-on-ubuntu-16-04) (As mentioned in the tutorial, do not install Gunicorn using `virtualenv`, just install everything globally)
